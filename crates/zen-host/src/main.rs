...
fn validate_mcp_config(config: &McpClientConfig) -> Result<()> {
    ...
    for server in &config.servers {
        ...
        match &server.source {
            zen_core::McpServerSource::Http { url, .. }
            | zen_core::McpServerSource::WebSocket { url, .. } => {
            ...
            zen_core::McpServerSource::Process { command, .. } => {
                if command.is_empty() {
...
async fn main() -> Result<()> {
    ...
    warn!(
        "zentrix-server is deprecated. Please use `zentrix serve` from zentrix-cli instead."
    );
    ...
    if args.interactive_mode {
        interactive_mode(zentrix, search_embedding_model.is_some(), args.thinking).await;
        return Ok(());
    }
    ...
    let mcp_port = if let Some(port) = args.mcp_port {
        ...
        let mcp_server = mcp_server::create_http_mcp_server(zentrix.clone(), host, port);
    ...
    let oai_port = if let Some(port) = args.port {
        ...
        let app = ZentrixServerRouterBuilder::new()
