...
use zen_core::{
    Constraint, zentrix, NormalRequest, Request, RequestMessage, Response, SamplingParams,
};
...
pub async fn embeddings(
        ...
        Inputs::Prompt(prompts) => {
            ...
            for (index, result) in results.into_iter().enumerate() {
                match result {
                    ...
                    Err(e) => {
                        zentrix::maybe_log_error(state.clone(), e.as_ref());
    ...
    zentrix::maybe_log_response(state.clone(), &response);
