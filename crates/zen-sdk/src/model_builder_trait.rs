...
impl MultiModelBuilder {
    ...
    pub async fn build(self) -> anyhow::Result<Model> {
        ...
        // Build the first model to create the initial zentrix instance
        ...
        // Create the ZenBuilder for the first model
        let mut runner_builder = zen_core::ZenBuilder::new(
            pipeline,
            scheduler_config,
        ...
        let zentrix = runner_builder.build().await;
        ...
        // Otherwise, the first model is already the default (set by zen_sdk::new)

        Ok(Model::new(zentrix))
    }
}
...
pub(crate) async fn build_pipeline_from_text_loader(
    ...
    loader: Box<dyn zen_core::Loader>,
) -> anyhow::Result<(Arc<Mutex<dyn Pipeline>>, SchedulerConfig, AddModelConfig)> {
    use zen_core::*;
    ...
    let device_map_setting =
        builder
            .device_mapping
            .clone()
            .unwrap_or(zen_core::DeviceMapSetting::Auto(
                zen_core::AutoDeviceMapParams::default_text(),
            ));
...
pub async fn build_model_from_pipeline(
    pipeline: Arc<Mutex<dyn zen_core::Pipeline>>,
