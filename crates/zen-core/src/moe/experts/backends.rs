...
impl CutileExpertsWeights {
    pub(super) fn from_checkpoint(ckpt: &ExpertCheckpoint) -> Result<CutileExpertsWeights> {
        ...
        zen_kernels_quant::cutile::register_moe_shape(
            w.gate_up.clone(),
            w.down.clone(),
...
impl FastExpertsWeights {
    ...
    pub(super) fn from_uqff(
        cfg: &MoEExpertsConfig,
        experts_vb: &ShardedVarBuilder,
        comm: &Arc<zen_kernels_quant::Comm>,
        ...
        if world_size > 1 && !sharded {
            zen_kernels_quant::log::once_log_warn(
...
impl CutileExpertsWeights {
    pub(super) fn forward_impl(
        ...
        let cfg = zen_kernels_quant::cutile::get_default_config(num_tokens, num_experts);
        ...
        let (sids, eids, ntpp, em) = zen_kernels_quant::moe::cuda::moe_align(
            ti_slice,
            num_tokens,
            num_experts,
            topk,
        ...
        let ic1 = zen_kernels_quant::cutile::cutile_grouped_gemm(
            forward.xs_flat,
            &self.w.gate_up,
            &sids,
            &eids,
        ...
        let ic2 = zen_kernels_quant::moe::cuda::gelu_tanh_and_mul(&ic1, inter, dev)?;
        ...
        let ic3 = zen_kernels_quant::cutile::cutile_grouped_gemm(
            &ic2,
            &self.w.down,
            &sids,
            &eids,
        ...
        zen_kernels_quant::moe::cuda::moe_sum_bf16(&ic3, num_tokens, topk, dev)
    }
}
...
impl FastExpertsWeights {
    ...
    pub(super) fn forward_decode(
        ...
        let act_type = match config.act {
            Activation::GeluPytorchTanh => zen_kernels_quant::ACT_GELU_PYTORCH_TANH,
            Activation::Silu | Activation::Swish => zen_kernels_quant::ACT_SILU,
            _ => return Ok(None),
        };

        // SAFETY: tw_ptr is a valid device pointer obtained from the topk_weights tensor above.
        let result = unsafe {
            zen_kernels_quant::indexed_moe_fused_decode(
                &gate_qt,
                &up_qt,
                &down_qt,
                forward.xs_flat,
    ...
    pub(super) fn forward_grouped(
        ...
        let (expert_bounds, sorted_token_ids, sorted_source_ids) =
            zen_kernels_quant::moe_dispatch_build(
                ti_u32_slice,
                total_assignments,
                num_experts,
                topk,
        ...
        let use_mmq_gate_up =
            gate_qt.dtype() == up_qt.dtype() && zen_kernels_quant::supports_mmq(gate_qt.dtype());

        let (gate, up, down_input_dim1) = if use_mmq_gate_up {
            let (gate, up) = zen_kernels_quant::grouped_moe_mmq_pair(
                &gate_qt,
                &up_qt,
                forward.xs_flat,
                &sorted_source_ids,
            ...
            let (input_q8, k, k_padded) =
                zen_kernels_quant::quantize_input_q8_1(forward.xs_flat, dev)?;

            // Gate projection using pre-quantized input
            let gate = zen_kernels_quant::grouped_moe_gemm_prequantized(
                &gate_qt,
                &input_q8,
                k,
                k_padded,
            ...
            let up = zen_kernels_quant::grouped_moe_gemm_prequantized(
                &up_qt,
                &input_q8,
                k,
                k_padded,
        ...
        let down = if let (true, Some(glu_activation)) = (
            zen_kernels_quant::supports_mmq(down_qt.dtype()),
            glu_activation,
        ) {
            let down_assignments = zen_kernels_quant::grouped_moe_mmq_from_glu_pair(
                &down_qt,
                &gate,
                &up,
                &sorted_token_ids,
            ...
            if forward.original_dtype == DType::BF16 {
                unsafe {
                    zen_kernels_quant::moe_weighted_reduce_flat_bf16(
                        &down_assignments,
                        tw_ptr,
                        forward.shape.num_tokens,
                        topk,
                ...
                unsafe {
                    zen_kernels_quant::moe_weighted_reduce_flat(
                        &down_assignments,
                        tw_ptr,
                        forward.shape.num_tokens,
                        topk,
            ...
            let (down_input_q8, down_k, down_k_padded) =
                zen_kernels_quant::quantize_input_q8_1(&activated, dev)?;

            // Down projection with topk_weights + atomicAdd
            zen_kernels_quant::grouped_moe_gemm_prequantized(
                &down_qt,
                &down_input_q8,
                down_k,
                down_k_padded,
