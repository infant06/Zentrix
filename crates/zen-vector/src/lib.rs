//! ZenVector implementation for vector search.
//!
...
//!
//! ```no_run
//! use zen_vector::ZenVectorEngine;
//!
//! // 1536-dim vectors compressed to 4 bits per coordinate.
//! let mut index = ZenVectorEngine::new(1536, 4).unwrap();
//!
...
//! index.write("index.zv").unwrap();
//! let loaded = ZenVectorEngine::load("index.zv").unwrap();
//! ```
//!
//! # Concurrent search
//!
...
//! without locking. [`ZenVectorEngine::prepare`] can be called once
...
///
/// Materialised lazily by [`ZenVectorEngine::search`] on first call
/// and re-materialised when [`ZenVectorEngine::add`] resets the
/// enclosing `OnceLock`.
struct BlockedCache {
    data: Vec<u8>,
    n_blocks: usize,
...
impl ZenVectorEngine {
    ...
    pub fn add(&mut self, vectors: &[f32]) {
        let dim = self.dim.expect(
            "ZenVectorEngine dim is not set; use add_2d(vectors, dim) on the \
             first add or construct via ZenVectorEngine::new(dim, bit_width)",
        );
    ...
    ///
    /// Call [`ZenVectorEngine::prepare`] once after `add`/`load` to
    ...
    /// [`ZenVectorEngine::load`] or after a batch of [`add`] calls.
    ///
...
mod from_parts_tests {
    //! Unit tests for `ZenVectorEngine::from_parts` length-invariant
    ...
    fn from_parts_panics_on_packed_codes_length_mismatch() {
        ...
        let _ = ZenVectorEngine::from_parts(
            Some(64),
            4,
            2,
            vec![0u8; 32],
    ...
    fn from_parts_accepts_lazy_uncommitted() {
        ...
        let idx = ZenVectorEngine::from_parts(
            None,
            4,
            0,
            Vec::new(),
