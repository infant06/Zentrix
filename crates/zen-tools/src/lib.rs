...
use zen_isolate::{NetworkMode, Sandbox, SandboxPolicy};
...
impl From<zen_protocol::CodeExecutionPermission> for CodeExecutionPermission {
    fn from(value: zen_protocol::CodeExecutionPermission) -> Self {
        match value {
            zen_protocol::CodeExecutionPermission::Auto => Self::Auto,
            zen_protocol::CodeExecutionPermission::Ask => Self::Ask,
            zen_protocol::CodeExecutionPermission::Deny => Self::Deny,
        }
    }
}
...
async fn sandbox_for_config(
    ...
    let (sandbox, mut policy): (Arc<dyn Sandbox>, SandboxPolicy) =
        match config.sandbox_policy.clone() {
            Some(policy) => (Arc::from(zen_isolate::detect()), policy),
            None => (
                Arc::from(zen_isolate::null()),
...
impl CodeExecutionManager {
    ...
    pub fn network_mode(&self) -> Option<zen_isolate::NetworkMode> {
    ...
    pub fn effective_protection(&self) -> zen_isolate::EffectiveProtection {
    ...
    pub fn get_tool_callbacks(&self, input_modalities: &[InputModality]) -> ToolCallbacksWithTools {
        ...
        let execute_callback: Arc<zen_protocol::MultimodalToolCallback> = Arc::new(
            move |func: &CalledFunction, tc: &zen_protocol::ToolCallContext| {
        ...
        let reset_callback: Arc<zen_protocol::ToolCallback> = Arc::new(
            move |_func: &CalledFunction, tc: &zen_protocol::ToolCallContext| {
                let sessions = Arc::clone(&sessions);
                let ctx = ctx.clone();
        ...
        let read_file_stub: Arc<zen_protocol::ToolCallback> = Arc::new(|_, _| {
            Err(anyhow::anyhow!(
        ...
        let list_files_stub: Arc<zen_protocol::ToolCallback> = Arc::new(|_, _| {
            Err(anyhow::anyhow!(
...
fn denied_by_permission(
    ctx: &SpawnCtx,
    tool_ctx: &zen_protocol::ToolCallContext,
    session_id: &str,
    code: &str,
    outputs: &[String],
) -> Option<String> {
    ...
    let reason = match permission {
        ...
        CodeExecutionPermission::Ask => {
            ...
            if let Some(notifier) = &tool_ctx.code_execution_approval_notifier {
                notifier(zen_protocol::CodeExecutionApprovalRequest {
                    approval_id,
