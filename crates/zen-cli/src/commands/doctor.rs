...
use zen_core::{run_doctor as run_doctor_report, DoctorStatus};
...
pub fn run_doctor(json: bool) -> Result<()> {
    ...
    let cuda_feature = if cfg!(feature = "cuda") { "true" } else { "false" };
    println!("[INFO] CUDA feature enabled: {}", cuda_feature);
    let hw = zen_hardware::detect_hardware();
    let has_cuda = hw.accelerators.iter().any(|a| a.kind == zen_hardware::AcceleratorType::Nvidia);
    let cuda_runtime = if has_cuda { "detected" } else { "none" };
    println!("[INFO] CUDA runtime available: {}", cuda_runtime);
    
    let best_accel = hw.accelerators.iter().filter(|a| a.kind != zen_hardware::AcceleratorType::CpuOnly).max_by_key(|a| a.vram_available);
    if let Some(a) = best_accel {
        println!("[INFO] Accelerator: {} ({:.1} GB VRAM)", a.name, a.vram_available as f64 / 1e9);
        println!("[INFO] Recommendation: Can run models up to {:.1} GB fully in VRAM", (a.vram_available as f64 * 0.8) / 1e9);
    } else {
        println!("[INFO] Accelerator: None");
        println!("[INFO] Recommendation: CPU-only inference using RAM");
    }
