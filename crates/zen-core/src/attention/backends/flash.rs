...
fn flash_attn_v2(
        ...
        if let Some(params) = flash_params {
                ...
                params.cumulative_seqlens_q.get(&q.device().location())
            {
                ...
                if let Some(softcap) = sdpa_params.softcap {
                    return zen_kernels_flash::flash_attn_varlen_alibi_windowed_softcap(
                        &q,
                        &k,
                        &v,
                        None,
                    ...
                    return zen_kernels_flash::flash_attn_varlen_windowed(
                        &q,
                        &k,
                        &v,
                        cumulative_seqlens_q,
    ...
    if let Some(softcap) = sdpa_params.softcap {
        ...
        zen_kernels_flash::flash_attn_windowed(
            q,
            k,
            v,
            sdpa_params.softmax_scale,
