...
mod tests {
    ...
    fn workdir_gets_read_write_rules() {
        let profile = render(&SandboxPolicy {
            session_workdir: Some(PathBuf::from("/tmp/zentrix workdir")),
            ..SandboxPolicy::default()
        });

        assert!(profile.contains("(allow file-read* (literal \"/tmp/zentrix workdir\"))"));
        assert!(profile.contains("(allow file-write* (literal \"/tmp/zentrix workdir\"))"));
        assert!(profile.contains("(allow file-write* (subpath \"/tmp/zentrix workdir\"))"));
        assert!(
            !profile.contains("(allow file-map-executable (literal \"/tmp/zentrix workdir\"))")
        );
        assert!(
            !profile.contains("(allow file-map-executable (subpath \"/tmp/zentrix workdir\"))")
        );
    }
