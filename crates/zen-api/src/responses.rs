...
use either::Either;
use zen_core::{ChatCompletionResponse, zentrix, Request, Response};
...
use crate::{
    ...
    types::{ExtractedZentrixState, OnDoneCallback, SharedZentrixState},
...
pub struct RequestContext {
    ...
    pub tools: Option<Vec<zen_core::Tool>>,
    /// Tool choice configuration from the request
    pub tool_choice: Option<zen_core::ToolChoice>,
...
pub enum IncludeOption {
    /// Include file search results (not currently supported by Zentrix)
    #[serde(rename = "file_search_call.results")]
    FileSearchCallResults,
    /// Include message input image URLs in the response
    #[serde(rename = "message.input_image.image_url")]
    MessageInputImageUrl,
    /// Include computer call output image URLs (not currently supported by Zentrix)
...
pub struct OpenResponsesCreateRequest {
    ...
    /// will return an error as Zentrix does not support disabling parallel tool calls.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub parallel_tool_calls: Option<bool>,
    ...
    /// as Zentrix does not support limiting the number of tool calls.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub max_tool_calls: Option<usize>,
    ...
    // ===== Zentrix extensions (non-standard) =====
    ...
    /// Repetition penalty (Zentrix extension)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub repetition_penalty: Option<f32>,

    /// Top-k sampling (Zentrix extension)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub top_k: Option<usize>,

    /// Grammar for constrained generation (Zentrix extension)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub grammar: Option<crate::openai::Grammar>,

    /// Min-p sampling (Zentrix extension)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub min_p: Option<f64>,

    /// DRY multiplier (Zentrix extension)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub dry_multiplier: Option<f32>,

    /// DRY base (Zentrix extension)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub dry_base: Option<f32>,

    /// DRY allowed length (Zentrix extension)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub dry_allowed_length: Option<usize>,

    /// DRY sequence breakers (Zentrix extension)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub dry_sequence_breakers: Option<Vec<String>>,

    /// Web search options (Zentrix extension)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub web_search_options: Option<zen_core::WebSearchOptions>,
}
        ...
        match self.rx.poll_recv(cx) {
            Poll::Ready(Some(resp)) => match resp {
                ...
                Response::Chunk(chat_chunk) => {
                    ...
                    zentrix::maybe_log_response(self.state.clone(), &chat_chunk);
    ...
    if let Some(false) = oairequest.parallel_tool_calls {
        anyhow::bail!(
            "parallel_tool_calls=false is not supported. \
             Zentrix does not currently support disabling parallel tool calls."
        );
    }
    ...
    if oairequest.max_tool_calls.is_some() {
        anyhow::bail!(
            "max_tool_calls is not supported. \
             Zentrix does not currently support limiting the number of tool calls."
        );
    }
...
pub async fn get_response(
    State(_state): ExtractedZentrixState,
