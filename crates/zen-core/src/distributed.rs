...
pub use zen_kernels_quant::distributed::{use_nccl, use_ring};
use zen_kernels_quant::{RingConfig, ShardedVarBuilder};
...
pub(crate) const IS_DAEMON_FLAG: &str = "__zenllm_DAEMON_INTERNAL";
...
pub(crate) fn ipc_name() -> anyhow::Result<Name<'static>> {
    let printname = "zenllm_daemon.sock";
...
pub(crate) fn prepare_distributed_mapper<T: DeviceMappedModelLoader + IsqModelLoader + ?Sized>(
    ...
    let global_world_size = if let Ok(x) = std::env::var("zenllm_MN_GLOBAL_WORLD_SIZE") {
        usize::from_str(&x).context("zenllm_MN_GLOBAL_WORLD_SIZE")?
    } else {
        // global world size is always >= local world size
        std::cmp::max(
            zen_kernels_quant::distributed::get_global_tp_size_from_devices()?,
            local_world_size,
        )
    };

    let use_multi_node = std::env::var("zenllm_MN_GLOBAL_WORLD_SIZE").is_ok();
    if use_multi_node {
        info!("zenllm_MN_GLOBAL_WORLD_SIZE is set, entering multi-node.");
    }
    ...
    let local_rank = if let Ok(payload) = env::var(IS_DAEMON_FLAG) {
        ...
        id = zen_kernels_quant::Id::new();
        let num_ranks = zen_kernels_quant::distributed::get_global_tp_size_from_devices()?;
    ...
    if use_multi_node {
        if let Ok(n_nodes) = env::var("zenllm_MN_HEAD_NUM_WORKERS") {
            let n_nodes = usize::from_str(&n_nodes).context("zenllm_MN_HEAD_NUM_WORKERS")?;
            info!("Head node managing {n_nodes} workers.");
            let Ok(port) = env::var("zenllm_MN_HEAD_PORT") else {
                anyhow::bail!("Got zenllm_MN_HEAD_NUM_WORKERS, expected zenllm_MN_HEAD_PORT");
            };
            info!("Head node initializing connection on {port}.");
            let server = zen_kernels_quant::Server::new(
                &format!("0.0.0.0:{port}"),
                n_nodes,
                local_world_size,
            )?;
        ...
        } else if let Ok(addr) = env::var("zenllm_MN_WORKER_SERVER_ADDR") {
            info!("Worker node connecting to {addr}.");
            let client = zen_kernels_quant::Client::new(addr.parse()?, local_world_size)?;

            id = client.receive_id()?;
        }
    }

    let rank_offset = if env::var("zenllm_MN_WORKER_SERVER_ADDR").is_ok() {
        let Ok(node_id) = env::var("zenllm_MN_WORKER_ID") else {
            anyhow::bail!("Got zenllm_MN_WORKER_SERVER_ADDR, expected zenllm_MN_WORKER_ID");
        };
        let node_id = usize::from_str(&node_id).context("zenllm_MN_WORKER_ID")?;
    ...
    let comm = zen_kernels_quant::Comm::from_device(
        id,
        device,
