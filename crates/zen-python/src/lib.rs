...
use zen_core::{
    ...
    LlguidanceGrammar, Loader, MemoryGpuConfig, zentrix as ZentrixRuntime, ZenBuilder,
    ...
    Request as _Request, RequestMessage, Response, ResponseOk, RuntimeMode, SamplingParams, SchedulerConfig,
...
use zen_hardware::{HardwareDetector};
use zen_core::{
    CalledFunction, SearchCallback, SearchFunctionParameters, SearchResult, ToolCallback,
    ToolCallbacks,
};
use zen_protocol::{McpClientConfig, McpServerConfig, McpServerSource};
...
fn get_device(seed: Option<u64>) -> &'static Result<Device> {
    DEVICE.get_or_init(|| {
        let device = if zen_core::distributed::use_nccl() {
            Device::Cpu
        } else {
...
/// Parse the runtime mode string passed from Python into a [`RuntimeMode`].
///
/// Accepts kebab-case (the canonical serde form), snake_case, and a handful of
/// common aliases (e.g. `cpu`, `gpu`, `flex`). Unknown values fall back to
/// [`RuntimeMode::Auto`] with a warning on stderr.
fn parse_runtime_mode(mode: &Option<String>) -> RuntimeMode {
    match mode.as_deref().map(|s| s.trim().to_lowercase()) {
        None => RuntimeMode::Auto,
        Some(m) => match m.replace('_', "-").as_str() {
            "" | "auto" => RuntimeMode::Auto,
            "cpu" | "cpu-only" => RuntimeMode::CpuOnly,
            "gpu" | "gpu-only" | "cuda" => RuntimeMode::GpuOnly,
            "flex" | "flexload" | "flex-load" => RuntimeMode::FlexLoad,
            "partial" | "partial-offload" => RuntimeMode::PartialOffload,
            "multigpu" | "multi-gpu" => RuntimeMode::MultiGpu,
            other => {
                eprintln!("[zentrix] Unknown runtime mode '{other}', defaulting to 'auto'");
                RuntimeMode::Auto
            }
        },
    }
...
/// Select the compute device based on the requested [`RuntimeMode`].
///
/// `CpuOnly` forces execution on the host CPU, which bypasses the CUDA/Metal
/// kernels entirely (useful to sidestep unsupported quantized GPU kernels).
/// Every other mode uses the accelerator when one is available, matching the
/// historical `cuda_if_available` behaviour.
fn get_device_for_mode(seed: Option<u64>, mode: RuntimeMode) -> Result<Device> {
    match mode {
        RuntimeMode::CpuOnly => {
...
fn wrap_tool_callback(cb: PyObject) -> Arc<ToolCallback> {
    Arc::new(
        move |func: &CalledFunction, _ctx: &zen_core::ToolCallContext| {
            Python::with_gil(|py| {
...
impl Runner {
    ...
    fn new(
        ...
        let runtime_mode = parse_runtime_mode(&mode);
        ...
        let device = get_device_for_mode(seed, runtime_mode).map_err(|e| PyApiErr::from(&e))?;
        ...
        let no_paged_attn = if device.is_cuda() || zen_core::distributed::use_nccl() {
            no_paged_attn
        ...
        if let Some(mtp_model) = mtp_model {
            pipeline
                .blocking_lock()
                .attach_speculative(SpeculativeConfig::Mtp(zen_core::MtpConfig::new(
                    mtp_model,
                    mtp_n_predict,
                )))
        ...
        let mut builder =
            ZenBuilder::new(pipeline, scheduler_config, false, search_embedding_model);
        if let Some(cb) = cb {
        ...
        if let Some(code_exec) = code_execution_config {
            ...
            #[cfg(not(feature = "code-execution"))]
            {
                ...
                return Err(util::PyApiErr(PyValueError::new_err(
                    "code_execution_config requires the 'code-execution' feature; rebuild zentrix with `--features code-execution`",
                )));
            }
        }
        let rt = Runtime::new().expect("Failed to create Runner::new runtime");
        let zentrix = rt.block_on(async {
            builder
    ...
    fn send_chat_completion_request(
        ...
        Python::with_gil(|py| {
            ...
            let tool_choice = request.tool_choice.as_ref().map(|x| match x {
                ToolChoice::Auto => zen_core::ToolChoice::Auto,
                ToolChoice::NoTools => zen_core::ToolChoice::None,
            });
    ...
    fn send_embedding_request(
        ...
        Python::with_gil(|py| {
            ...
            py.allow_threads(move || -> std::result::Result<Vec<Vec<f32>>, String> {
                ZentrixRuntime::maybe_log_request(runner.clone(), debug_repr);
                let sender = runner
    ...
    fn import_session(
        ...
        let session: zen_core::SerializedSession = serde_json::from_str(&session_json)
    ...
    fn find_file(&self, file_id: String) -> Option<zen_core::File> {
...
fn detect_hardware() -> PyResult<String> {
    ...
    serde_json::to_string(&hw).map_err(|e| PyApiErr::from(anyhow::anyhow!(e)).into())
}

#[pymodule]
fn zentrix(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    initialize_logging();
    ...
    m.add_class::<zen_core::File>()?;
    m.add_class::<zen_core::FileSource>()?;
    m.add_class::<ToolChoice>()?;
    m.add_class::<SpeechGenerationResponse>()?;
    m.add_class::<SpeechLoaderType>()?;

    m.add_class::<zen_core::ResponseMessage>()?;
    m.add_class::<zen_core::Delta>()?;
    m.add_class::<zen_core::ResponseLogprob>()?;
    m.add_class::<zen_core::Logprobs>()?;
    m.add_class::<zen_core::Choice>()?;
    m.add_class::<zen_core::ChunkChoice>()?;
    m.add_class::<zen_core::Usage>()?;
    m.add_class::<zen_core::AgenticToolCallRecord>()?;
    m.add_class::<zen_core::ChatCompletionResponse>()?;
    m.add_class::<zen_core::ChatCompletionChunkResponse>()?;
    m.add_class::<zen_core::CompletionChoice>()?;
    m.add_class::<zen_core::CompletionResponse>()?;
    m.add_class::<zen_core::TopLogprob>()?;
    m.add_class::<zen_core::ModelDType>()?;
    m.add_class::<zen_core::ImageGenerationResponseFormat>()?;
    m.add_class::<McpServerSourcePy>()?;
    m.add_class::<McpServerConfigPy>()?;
    m.add_class::<McpClientConfigPy>()?;
    m.add_class::<zen_core::WebSearchOptions>()?;
    m.add_class::<zen_core::SearchContextSize>()?;
    m.add_class::<zen_core::WebSearchUserLocation>()?;
    m.add_class::<zen_core::ApproximateUserLocation>()?;
    Ok(())
}
