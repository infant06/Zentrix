...
use zen_kernels_quant::{IsqBits, IsqType, TrackedModule, UqffTensor};
use regex::Regex;
...
fn write_uqff_type(
    ...
    for version in zen_kernels_quant::uqff_version_tensors() {
    ...
    let handles = zen_kernels_quant::requantize_tracked(
        layers,
        ty,
        zen_kernels_quant::RequantizeResults::CpuStaged,
        |m| m.ty.unwrap_or(ty),
        &|key| imatrix.get(key).cloned(),
    )?;
    let guard = zen_kernels_quant::QuantizeOntoGuard::new();
...
fn gguf_imatrix_name(key: &str) -> Option<String> {
    ...
    let index = zen_kernels_quant::layer_index_from_prefix(key)?;
...
pub(crate) fn load_imatrix_map(
    ...
    if path.extension().is_some_and(|ext| ext == "cimatrix") {
        ...
        return Ok(zen_kernels_quant::CollectedImatrixData::load_imatrix(path)?.0);
    }
