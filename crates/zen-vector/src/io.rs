//! Read/write zen-vector index files.
//!
//! Two formats live here:
//! * `.tv` — [`ZenVectorIndex`](crate::ZenVectorIndex) — 4-byte magic
...
//! ## Format versioning
//!
//! Both formats are at version 3 as of zen-vector 0.6.x (TQ+ per-coord
//! calibration). Version 2 (zen-vector 0.4.4 .. 0.6.0) is loaded transparently
...
//! up the new calibration. Version 1 (zen-vector ≤ 0.4.3) is incompatible
...
//! lets us detect either a current file or "looks like a v1 zen-vector
//! file" cleanly.
...
const REBUILD_HINT: &str =
    "Rebuild this index from the source vectors using zen-vector 0.4.4 or later \
...
pub fn load(path: impl AsRef<Path>) -> io::Result<CoreLoad> {
    ...
    if &magic != TV_MAGIC {
        ...
        // message; otherwise treat it as a non-zen-vector file.
        if (2..=4).contains(&magic[0]) {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "this .tv file was written by zen-vector ≤ 0.4.3 (format \
                     version 1). It is incompatible with zen-vector 0.4.4+ \
        ...
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "not a zen-vector .tv file: wrong magic",
        ));
    }
...
pub fn load_id_map(
    ...
    if version[0] == 1 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "this .tvim file was written by zen-vector ≤ 0.4.3 (format \
                 version 1). It is incompatible with zen-vector 0.4.4+ \
                 because the per-vector scalar's meaning changed. {}",
                REBUILD_HINT,
            ),
