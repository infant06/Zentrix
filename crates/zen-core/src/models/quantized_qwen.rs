...
use zen_kernels_quant::{GgufMatMul, QuantMethod, QuantMethodConfig};
...
impl ModelWeights {
    pub fn forward(
        ...
        // The final norm, logits extraction, and output head live on the primary
        // (non-mapped) device. When tail layers are offloaded to another device
        // (e.g. CPU during partial offload), `layer_in` is left on that device,
        // so move it back to the primary device to avoid a device mismatch in
        // the final RMS norm.
