...
pub trait RequestLike {
    ...
    fn code_execution_permission(&self) -> Option<zen_core::CodeExecutionPermission> {
        None
    }
    fn agent_permission(&self) -> Option<zen_core::AgentPermission> {
        None
    }
    fn agent_approval_handler(&self) -> Option<zen_core::AgentToolApprovalHandler> {
        None
    }
...
pub struct RequestBuilder {
    ...
    code_execution_permission: Option<zen_core::CodeExecutionPermission>,
    agent_permission: Option<zen_core::AgentPermission>,
    agent_approval_handler: Option<zen_core::AgentToolApprovalHandler>,
...
impl RequestBuilder {
    ...
    pub fn with_agent_approval_callback(
        mut self,
        callback: zen_core::AgentToolApprovalCallback,
    ) -> Self {
        self.agent_approval_handler = Some(zen_core::AgentToolApprovalHandler::from_sync(
            callback,
        ));
        self
    }
    ...
    where
        F: Fn(zen_core::AgentToolApproval) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = zen_core::AgentToolApprovalDecision> + Send + 'static,
    {
        let callback = Arc::new(move |approval| {
            Box::pin(callback(approval)) as zen_core::AgentToolApprovalFuture
        });
        self.agent_approval_handler = Some(zen_core::AgentToolApprovalHandler::from_async(
            callback,
        ));
        self
    }
