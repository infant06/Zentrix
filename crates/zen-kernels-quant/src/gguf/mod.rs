...
/// Returns true when `ZEN_DISABLE_FAST_QMATMUL` is set to a non-empty, non-"0"
/// value. When enabled, GGUF matmuls skip the custom CUDA MMVQ/MMQ kernels and
/// use candle's built-in quantized matmul instead.
#[cfg(feature = "cuda")]
fn fast_qmatmul_disabled() -> bool {
    std::env::var("ZEN_DISABLE_FAST_QMATMUL")
...
impl GgufMatMul {
    pub fn isq_type_from_uqff_dtype(dtype: u32) -> Result<IsqType> {        IsqType::try_from(ggml_dtype_from_uqff_code(dtype)?)
    }
    ...
    fn uses_fast_mmvq(&self) -> bool {
        // Escape hatch: set ZEN_DISABLE_FAST_QMATMUL=1 to bypass the custom
        // MMVQ/MMQ GGUF kernels and fall back to candle's built-in quantized
        // matmul. Useful to isolate/avoid kernel faults on some GPUs.
        if fast_qmatmul_disabled() {
            return false;
        }
        matches!(
            &self.w,
