...
use zen_kernels_quant::log::once_log_info;
use zen_kernels_quant::IsqType;
...
pub struct NormalPipeline {
    ...
    tracked_modules: Vec<zen_kernels_quant::TrackedModule>,
}
...
impl NormalLoaderBuilder {
    pub fn new(
        ...
        Self {
            ...
            jinja_explicit,
            no_kv_cache,
            mode: crate::RuntimeMode::Auto,
            ..Default::default()
        }
    }

    pub fn with_mode(mut self, mode: crate::RuntimeMode) -> Self {
        self.mode = mode;
        self
    }
...
impl Loader for NormalLoader {
    ...
    fn load_model_from_path(
        ...
        let use_nccl = zen_kernels_quant::distributed::use_nccl();
        ...
        let uqff_reader = if let Some(from_uqff) = &*self.from_uqff.read().unwrap() {
            Some(Arc::new(zen_kernels_quant::UqffReader::open(from_uqff)?))
        } else {
            None
        };
        ...
        if plan.capture == zen_kernels_quant::IsqCaptureMode::CaptureMatches {
