...
pub async fn run_bench(
    ...
    runtime_opts: crate::args::RuntimeOptions,
    ...
    let cpu = device_opts.cpu;
    let mut device_layers = device_opts.device_layers.clone();

    if runtime_opts.gpu_layers != "auto" {
        if runtime_opts.gpu_layers == "all" {
            device_layers = Some(vec!["0:999".to_string()]);
        } else {
            if let Ok(num) = runtime_opts.gpu_layers.parse::<usize>() {
    ...
    let builder = ZentrixForServerBuilder::new()
    ...
    if warmup > 0 {
        ...
        for _ in 0..warmup {
            run_single_bench(&zentrix, 32, 16).await?;
        }
    ...
    #[cfg(feature = "cuda")]
    let cuda_profiler_range = std::env::var_os("zen_bench_CUDA_PROFILER_RANGE")
        .or_else(|| {
            // Fallback to legacy name for backward compatibility
            std::env::var_os("zenllm_BENCH_CUDA_PROFILER_RANGE").map(|v| {
                tracing::warn!(
                    "zenllm_BENCH_CUDA_PROFILER_RANGE is deprecated, use zen_bench_CUDA_PROFILER_RANGE"
                );
                v
            })
        })
    ...
    for i in 0..iterations {
        ...
        for (prompt_len, results) in prefill_results.iter_mut() {
            ...
            let usage = run_single_bench(&zentrix, *prompt_len, 1).await?;
        ...
        if gen_len > 0 {
            for (depth, results) in decode_results.iter_mut() {
                let usage = run_single_bench(&zentrix, *depth, gen_len).await?;
...
async fn run_single_bench(
    zentrix: &Arc<zen_core::ZentrixRuntime>,
    prompt_tokens: usize,
    gen_tokens: usize,
) -> Result<Usage> {
