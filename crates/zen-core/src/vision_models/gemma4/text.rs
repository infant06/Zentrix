...
impl Attention {
    ...
    fn new(
        ...
        let kv_shard = zen_kernels_quant::compute_kv_shard(num_kv_heads, head_dim, comm)?;
        ...
        Ok(Self {
            ...
            paged_attn,
            sdpa_params: SdpaParams {
                n_kv_groups: zen_kernels_quant::compute_n_kv_groups(
                    num_kv_heads,
...
impl DecoderLayer {
    ...
    fn new(
        ...
        {
            let gate = zen_kernels_quant::linear_no_bias(
                cfg.hidden_size,
                ple_dim,
                &None,
                mapper.set_device(layer_idx, vb.pp("per_layer_input_gate"), loading_isq),
            )?;
            let proj = zen_kernels_quant::linear_no_bias(
                ple_dim,
                cfg.hidden_size,
                &None,
    ...
    pub fn new(
        ...
        let (embed_tokens_per_layer, per_layer_model_projection, per_layer_projection_norm) =
            if ple_dim > 0 {
                ...
                let ple_proj = zen_kernels_quant::linear_no_bias(
                    cfg.hidden_size,
