...
use zen_multimodal::audio::AudioInput;
...
#[derive(Clone, Serialize, Deserialize)]
/// A normal request request to the `zentrix`.
...
pub struct NormalRequest {
    ...
    #[serde(skip)]
    pub code_execution_approval_notifier: Option<Arc<zen_protocol::CodeExecutionApprovalNotifier>>,
    #[serde(default)]
    pub agent_permission: Option<AgentPermission>,
    #[serde(skip)]
    pub agent_approval_handler: Option<AgentToolApprovalHandler>,
    #[serde(skip)]
    pub agent_approval_notifier: Option<Arc<zen_protocol::AgentToolApprovalNotifier>>,
...
pub enum Request {
    ...
    // Sending a terminate request causes the `run` function to return to the thread created in `zentrix::new`,
