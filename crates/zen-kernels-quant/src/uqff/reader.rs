...
impl UqffReader {
    pub fn open(paths: &[PathBuf]) -> Result<Self> {
        ...
        if !names.is_empty() && names.iter().all(|name| name.parse::<usize>().is_ok()) {
            candle_core::bail!(
                "Pre-1.0 UQFF artifacts are no longer supported; regenerate with `zentrix quantize`."
            );
        }
        if names.contains(super::UQFF_VERSION_MAJOR_KEY) {
            ...
            if major != super::UQFF_VERSION_MAJOR {
                candle_core::bail!(
                    "UQFF version {major}.{minor}.{patch} is incompatible with this build ({ours}); regenerate with `zentrix quantize`."
                );
            }
            // Same major, higher minor: the file may use additions this reader does not know.
            if minor > super::UQFF_VERSION_MINOR {
                candle_core::bail!(
                    "UQFF version {major}.{minor}.{patch} was written by a newer Zentrix than this build ({ours}); upgrade Zentrix."
                );
            }
        } else {
            candle_core::bail!(
                "UQFF artifact has no version tag (pre-1.0 file); regenerate with `zentrix quantize`."
            );
        }
