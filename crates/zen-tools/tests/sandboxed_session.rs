...
use zen_isolate::{NetworkMode, SandboxPolicy};
...
fn exec_json(
    callbacks: &std::collections::HashMap<String, zen_protocol::ToolCallbackWithTool>,
    session_id: &str,
    code: &str,
) -> String {
...
fn exec_json_with_permission(
    ...
    let exec = callbacks
        .get(zen_tools::EXECUTE_PYTHON_TOOL_NAME)
        .expect("execute_python tool registered");

    let func = CalledFunction {
        name: zen_tools::EXECUTE_PYTHON_TOOL_NAME.to_string(),
    ...
    match result {
        zen_protocol::ToolOutput::Multimodal { text, .. } => text,
        zen_protocol::ToolOutput::Text(t) => t,
    }
}
...
fn reset_json(
    ...
    let reset = callbacks
        .get(zen_tools::RESET_SESSION_TOOL_NAME)
        .expect("reset_python_session tool registered");

    let func = CalledFunction {
        name: zen_tools::RESET_SESSION_TOOL_NAME.to_string(),
        arguments: "{}".to_string(),
    };
    let ctx = ToolCallContext {
        session_id: Some(session_id.to_string()),
