...
use zen_kernels_quant::QuantizedConfig;
...
impl BackendChoice {
    pub(super) fn new(
        ...
        Self {
            ...
            immediate_isq: zen_kernels_quant::get_immediate_isq().is_some(),
            act,
        }
    }
}
...
impl MoEExpertsBackend {
    fn from_env() -> Option<Self> {
        let force = crate::utils::env::env_or_legacy(
            "ZENTRIX_MOE_BACKEND",
            "zenllm_MOE_BACKEND",
        )?;
...
fn cutile_arch_supported(device: &Device) -> bool {
    match device {
        Device::Cuda(dev) => zen_kernels_quant::cutile::device_supported(dev),
        _ => false,
    }
}
