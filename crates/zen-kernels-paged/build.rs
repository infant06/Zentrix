...
fn main() -> Result<()> {
    ...
    let out_file = if target.contains("msvc") {
        // Windows case
        build_dir.join("zenllmpagedattention.lib")
    } else {
        build_dir.join("libzenllmpagedattention.a")
    };
    builder
        .build_lib(out_file)
    ...
    println!("cargo:rustc-link-lib=zenllmpagedattention");
...
fn main() -> Result<(), String> {
    ...
    if skip_precompile {
        ...
        std::fs::write(out_dir.join("zenllm_paged_attention.metallib"), []).unwrap();
        std::fs::write(out_dir.join("zenllm_paged_attention_ios.metallib"), []).unwrap();
        std::fs::write(out_dir.join("zenllm_paged_attention_tvos.metallib"), []).unwrap();
        return Ok(());
    }
    ...
    fn compile(platform: Platform) -> Result<(), String> {
        ...
        let lib_name = match platform {
            Platform::MacOS => "zenllm_paged_attention.metallib",
            Platform::Ios => "zenllm_paged_attention_ios.metallib",
            Platform::TvOS => "zenllm_paged_attention_tvos.metallib",
        };
