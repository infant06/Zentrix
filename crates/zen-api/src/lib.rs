...
//! ## About
//!
//! This crate powers Zentrix server. It exposes the underlying functionality
//! allowing others to implement and extend the server implementation.
//!
//! ### Features
//! 1. Incorporate Zentrix server into another axum.rs project.
//! 2. Hook into the Zentrix server lifecycle.
//!
//! ### Example
//! ```no_run
//! use std::sync::Arc;
...
//! #[derive(Clone)]
//! pub struct AppState {
//!     pub zentrix_state: SharedZentrixState,
//!     pub db_create: fn(),
//! }
//!
//! #[tokio::main]
...
//!     };
//!
//!     let shared_zentrix = ZentrixForServerBuilder::new()
//!         .with_model(model)
...
//!         .unwrap();
//!
//!     let zentrix_base_path = "/api/zentrix";
//!
//!     let zentrix_routes = ZentrixServerRouterBuilder::new()
//!         .with_zentrix(shared_zentrix.clone())
//!         .with_include_swagger_routes(false)
//!         .with_base_path(zentrix_base_path)
//!         .build()
//!         .await
//!         .unwrap();
//!
//!     let zentrix_doc = get_openapi_doc(Some(zentrix_base_path));
//!     let mut api_docs = ApiDoc::openapi();
//!     api_docs.merge(zentrix_doc);
//!
//!     let app_state = Arc::new(AppState {
//!         zentrix_state: shared_zentrix,
...
//!         .nest(zentrix_base_path, zentrix_routes)
...
//!     let zentrix_state = state.zentrix_state.clone();
//!     let (tx, mut rx) = create_response_channel(None);
//!
//!     let (request, is_streaming) =
//!         match parse_request(oai_request, zentrix_state.clone(), tx, None, None, None).await {
//!             Ok(x) => x,
//!             Err(e) => return handle_error(zentrix_state, e.into()),
//!         };
//!
//!     dbg!(request.clone());
//!
//!     if let Err(e) = send_request(&zentrix_state, request).await {
//!         return handle_error(zentrix_state, e.into());
//!     }
//!
//!     if is_streaming {
...
//!             });
//!
//!         let streamer = create_streamer(rx, zentrix_state.clone(), Some(on_chunk), Some(on_done));
//!
//!         ChatCompletionResponder::Sse(streamer)
//!     } else {
//!         let response = process_non_streaming_response(&mut rx, zentrix_state.clone()).await;
//!
//!         match &response {
...
pub mod metrics;
pub mod zentrix_for_server_builder;
pub mod zentrix_server_router_builder;
pub mod openai;
pub mod openapi_doc;
pub mod responses;
