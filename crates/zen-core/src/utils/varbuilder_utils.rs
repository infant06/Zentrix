...
use zen_kernels_quant::{safetensors::MmapedSafetensors, ShardedSafeTensors, ShardedVarBuilder};
use regex::Regex;
...
const ZENTRIX_NO_MMAP: &str = "ZENTRIX_NO_MMAP";
const ZEN_NO_MMAP: &str = "ZEN_NO_MMAP";
...
pub(crate) fn from_mmaped_safetensors(
    ...
    let use_no_mmap = crate::utils::env::is_flag_enabled(ZENTRIX_NO_MMAP, ZEN_NO_MMAP);
        ...
        if !iter.is_empty() {
            for (load_name, key_name) in iter.into_iter().with_progress(is_silent) {
                if !make_dummy_predicate(&load_name) {
                    ...
                    // If making a dummy, don't add the tensor. `zen_kernels_quant` handles this!
