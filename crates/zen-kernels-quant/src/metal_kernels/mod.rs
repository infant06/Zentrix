...
#[cfg(target_os = "macos")]
const KERNELS: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/zen_kernels_quant.metallib"));
#[cfg(target_os = "ios")]
const KERNELS: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/zen_kernels_quant_ios.metallib"));
#[cfg(target_os = "tvos")]
const KERNELS: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/zen_kernels_quant_tvos.metallib"));
...
impl Kernels {
    ...
    #[allow(clippy::const_is_empty)] // KERNELS can be empty when zenllm_METAL_PRECOMPILE=0
    pub fn load_library(&self, device: &Device) -> Result<Library, MetalKernelError> {
        if let Some(lib) = LIBRARY.get() {
            ...
            let lib = if !KERNELS.is_empty() {
                ...
                // https://github.com/EricLBuehler/Zentrix/issues/1897
                ...
                // (e.g., when zenllm_METAL_PRECOMPILE=0)
