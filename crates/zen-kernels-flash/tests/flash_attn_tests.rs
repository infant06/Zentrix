...
fn flash_attn_acausal() -> Result<()> {
    ...
    let ys2 = {
        ...
        zen_kernels_flash::flash_attn(&q, &k, &v, 0.5, false)?.transpose(1, 2)?
    };
...
fn flash_attn_varlen_paged_mm_prefix_windowed() -> Result<()> {
    ...
    let ys_causal = zen_kernels_flash::flash_attn_varlen_paged_windowed(
        &q,
        &k_paged,
        &v_paged,
        &seqlens,
    ...
    let ys = zen_kernels_flash::flash_attn_varlen_paged_windowed(
        &q,
        &k_paged,
        &v_paged,
        &seqlens,
...
fn flash_attn_acausal_softcap() -> Result<()> {
    ...
    let ys2 = {
        ...
        let k = k.transpose(1, 2)?;
        let v = v.transpose(1, 2)?;
        zen_kernels_flash::flash_attn_alibi_windowed_softcap(
...
fn flash_attn_varlen() -> Result<()> {
    ...
    let ys = {
        ...
        zen_kernels_flash::flash_attn_varlen(
