//! Error types and conversions for the zentrix SDK.

use thiserror::Error;

/// Error type for the zentrix SDK.
///
...
pub enum Error {
    ...
    ModelError {
        ...
        partial_response: Option<Box<zen_core::ChatCompletionResponse>>,
    },
    ...
    /// Wraps the existing [`zen_core::ZenError`].
    #[error(transparent)]
    Management(#[from] zen_core::ZenError),
...
impl From<tokio::sync::mpsc::error::SendError<zen_core::Request>> for Error {
    /// Convert from a channel send error (mapped to [`Error::Channel`]).
    fn from(e: tokio::sync::mpsc::error::SendError<zen_core::Request>) -> Self {
        Error::Channel(e.to_string())
    }
}

impl From<Box<zen_core::ResponseErr>> for Error {
    /// Convert from a boxed engine response error.
    fn from(e: Box<zen_core::ResponseErr>) -> Self {
        match *e {
            zen_core::ResponseErr::InternalError(inner) => Error::Inference(inner),
            zen_core::ResponseErr::ValidationError(inner) => {
                Error::RequestValidation(inner.to_string())
            }
            zen_core::ResponseErr::ModelError(message, response) => Error::ModelError {
                message,
                partial_response: Some(Box::new(response)),
            },
            zen_core::ResponseErr::CompletionModelError(message, _) => Error::ModelError {
                message,
