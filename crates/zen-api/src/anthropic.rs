...
use zen_core::{
    ...
    zentrix, Request, RequestMessage, Response, TokenizationRequest, Tool, ToolChoice, ToolType,
...
use crate::{
    ...
    types::{ExtractedZentrixState, SharedZentrixState},
...
pub struct AnthropicMessagesRequest {
    ...
    pub files: Option<Vec<zen_core::RequestedFile>>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl AnthropicStreamer {
    fn new(rx: Receiver<Response>, state: SharedZentrixState) -> Self {
    ...
    fn enqueue_tool_use(&mut self, tool_call: &zen_core::ToolCallResponse) {
        self.stop_thinking_block();
        self.stop_text_block();
        let index = self.next_content_index;
        self.next_content_index += 1;
    ...
    fn handle_chunk(&mut self, chunk: ChatCompletionChunkResponse) {
        zentrix::maybe_log_response(self.state.clone(), &chunk);
...
fn create_streamer(rx: Receiver<Response>, state: SharedZentrixState) -> AnthropicMessagesSse {
...
mod tests {
    ...
    #[test]
    fn routes_zenllm_sampling_and_constraint_extensions() {
        let req: AnthropicMessagesRequest = serde_json::from_value(json!({
            "model": "default",
            "max_tokens": 32,
