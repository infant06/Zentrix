#!/usr/bin/env python3
"""
Build script for zentrix Python wheels.
...
PYPROJECT_PATH = REPO_ROOT / "zentrix-pyo3" / "pyproject.toml"
CARGO_MANIFEST = REPO_ROOT / "zentrix-pyo3" / "Cargo.toml"
...
def get_package_configs() -> dict[str, PackageConfig]:
    ...
    return {
        ...
        "zentrix-accelerate": PackageConfig(
            name="zentrix-accelerate",
            features=["accelerate"],
            supported_os=[OS.DARWIN],
            supported_arch=[Arch.AARCH64],
            requires_accelerator=None,  # Accelerate is always available on macOS
        ),
        "zentrix-mkl": PackageConfig(
            name="zentrix-mkl",
            features=["mkl"],
            supported_os=[OS.LINUX, OS.WINDOWS],
...
def get_features_for_base_package(plat: Platform) -> list[str]:
    """Get features for the 'zentrix' base package based on platform."""
...
def restore_pyproject_name() -> None:
    """Restore project.name to default 'zentrix'."""
    modify_pyproject_name("zentrix")
...
def build_wheel(
    ...
    # Determine features
    if package_config.name == "zentrix":
...
def _build_with_docker(features: list[str], output_dir: Path, plat: Platform) -> None:
    ...
    docker_cmd.extend(["zentrix-wheelmaker:latest"] + maturin_args)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build zentrix Python wheels",
...
Examples:
  ...
  python scripts/build_wheels.py --packages zentrix zentrix-cuda
