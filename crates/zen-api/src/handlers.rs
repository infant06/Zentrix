//! ## General Zentrix server route handlers.
...
use zen_core::{
    ...
    AutoTuneRequest, AutoTuneResult, zentrix, ZenError, ModelDType, ModelSelected,
...
pub async fn models(State(state): ExtractedZentrixState) -> Json<ModelObjects> {
...
pub async fn system_info() -> Json<zen_core::SystemInfo> {
    Json(collect_system_info())
}

pub async fn system_doctor() -> Json<zen_core::DoctorReport> {
    Json(run_doctor())
}
...
pub async fn re_isq(
    ...
    zentrix::maybe_log_request(state.clone(), repr.clone());
    ...
    match state.unload_model(&model_id) {
        ...
        Err(e) => {
            let (status, error) = match &e {
                ZenError::ModelNotFound(_) => (ModelStatus::NotFound, None),
                ZenError::ModelAlreadyUnloaded(_) => (ModelStatus::Unloaded, None),
                ZenError::NoLoaderConfig(_) => (ModelStatus::NoLoaderConfig, None),
...
pub async fn reload_model(
    ...
    match state.reload_model(&model_id).await {
        ...
        Err(e) => {
            let (status, error) = match &e {
                ZenError::ModelNotFound(_) => (ModelStatus::NotFound, None),
                ZenError::ModelReloading(_) => (ModelStatus::Reloading, None),
                ZenError::ModelAlreadyLoaded(_) => (ModelStatus::Loaded, None),
                ZenError::ReloadFailed(msg) => {
