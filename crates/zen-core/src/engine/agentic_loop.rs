...
fn is_read_file_tool(name: &str) -> bool {
    name == zen_tools::READ_FILE_TOOL_NAME
}
#[cfg(not(feature = "code-execution"))]
...
fn is_list_files_tool(name: &str) -> bool {
    name == zen_tools::LIST_FILES_TOOL_NAME
}
#[cfg(not(feature = "code-execution"))]
    ...
    let message = match ctx.agent_permission {
        ...
        AgentPermission::Ask => {
            ...
            if let Some(notifier) = &ctx.tool_call_ctx.agent_approval_notifier {
                notifier(zen_protocol::AgentToolApprovalRequest {
...
struct DispatchCtx<'a> {
    ...
    supports_vision: bool,
    supports_video: bool,
    tool_call_ctx: &'a zen_protocol::ToolCallContext,
    run_id: &'a str,
    turn: usize,
    session_id: &'a str,
...
pub(super) async fn agentic_loop(this: Arc<Engine>, mut request: NormalRequest) {
    ...
    let handle = tokio::spawn(async move {
        let tool_call_ctx = zen_protocol::ToolCallContext {
