...
use zen_kernels_quant::{IsqType, QuantMethod, TrackedModule};
...
pub(crate) struct IsqPlanInputs<'a> {
    ...
    pub topology_overrides: Vec<zen_kernels_quant::ImmediateIsqOverride>,
...
pub(crate) struct IsqLoadPlan {
    ...
    pub capture: zen_kernels_quant::IsqCaptureMode,
...
pub(crate) fn resolve_and_install_isq_plan(i: IsqPlanInputs<'_>) -> Result<IsqLoadPlan> {
    ...
    if allow_immediate_cli {
        ...
        let (pool, num_threads) = zen_kernels_quant::create_isq_thread_pool(immediate_ty);
        tracing::debug!("Using {num_threads} worker thread(s) for weight quantization.");
        zen_kernels_quant::set_immediate_isq_with_pool(
            immediate_ty,
...
fn capture_mode(has_write_uqff: bool, wants_imatrix: bool) -> zen_kernels_quant::IsqCaptureMode {
    if has_write_uqff {
        zen_kernels_quant::IsqCaptureMode::CaptureAll
    } else if wants_imatrix {
        zen_kernels_quant::IsqCaptureMode::CaptureMatches
    } else {
        zen_kernels_quant::IsqCaptureMode::Immediate
    }
}
...
pub(crate) fn requantize_and_swap(
    ...
    let handles = zen_kernels_quant::requantize_tracked(
        modules,
        pool_ty,
        zen_kernels_quant::RequantizeResults::Resident,
        ty_for,
        imatrix_for,
    )?;
