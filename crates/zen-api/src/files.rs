...
use zen_core::{File as CoreFile, FileContent};
use serde::Serialize;

use crate::types::{ExtractedZentrixState, SharedZentrixState};

const PURPOSE: &str = "agent_output";

/// OpenAI file metadata + Zentrix extensions (`format`, `mime_type`, `source`, `truncated`).
#[derive(Serialize)]
...
pub async fn get_file(State(state): ExtractedZentrixState, Path(id): Path<String>) -> Response {
...
pub async fn list_files(State(state): ExtractedZentrixState) -> Response {
...
fn serve_bytes(state: SharedZentrixState, id: &str) -> Result<Response, (StatusCode, String)> {
