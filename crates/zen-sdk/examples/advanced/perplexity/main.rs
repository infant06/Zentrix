...
//!
//! Run with: `cargo run --release --example perplexity -p zentrix`
...
use either::Either;
use zen_sdk::{
    cross_entropy_loss, parse_isq_value, Constraint, DType, Device, zentrix, ModelBuilder,
...
async fn process_chunk(runner: &zentrix, chunk: Vec<u32>) -> anyhow::Result<(Tensor, Vec<u32>)> {
    let (tx, mut rx) = channel(1);

    let request = Request::Normal(Box::new(NormalRequest {
        messages: zen_sdk::RequestMessage::CompletionTokens(chunk),
