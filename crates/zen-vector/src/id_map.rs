//! Stable external IDs on top of [`ZenVectorEngine`].
//!
//! [`ZenVectorEngine`] stores vectors positionally: calling `swap_remove`
...
use crate::io;
use crate::{AddError, ConstructError, ZenVectorEngine};

/// ID-addressed wrapper around [`ZenVectorEngine`].
pub struct IdMapIndex {
    inner: ZenVectorEngine,
...
impl IdMapIndex {
    ...
    pub fn new(dim: usize, bit_width: usize) -> Result<Self, ConstructError> {
        Ok(Self {
            inner: ZenVectorEngine::new(dim, bit_width)?,
            slot_to_id: Vec::new(),
    ...
    /// call. Propagates the same errors as [`ZenVectorEngine::new_lazy`].
    pub fn new_lazy(bit_width: usize) -> Result<Self, ConstructError> {
        Ok(Self {
            inner: ZenVectorEngine::new_lazy(bit_width)?,
            slot_to_id: Vec::new(),
            id_to_slot: HashMap::new(),
        })
    }
    ...
    /// [`ZenVectorEngine::add_2d`](crate::ZenVectorEngine::add_2d).
    ...
    /// otherwise. O(1) via the inner [`ZenVectorEngine::swap_remove`].
    ...
    /// seen an add yet (matches [`ZenVectorEngine::dim`] semantics).
    ...
    pub fn write(&self, path: impl AsRef<Path>) -> std::io::Result<()> {
        // Mirror ZenVectorEngine::write: dim=0 means lazy-uninitialized.
        io::write_id_map(
            path,
            self.inner.bit_width(),
    ...
    pub fn load(path: impl AsRef<Path>) -> std::io::Result<Self> {
        ...
        let inner = ZenVectorEngine::from_parts(
