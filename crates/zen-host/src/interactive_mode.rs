...
use zen_core::{
    ...
    ImageGenerationResponseFormat, MessageContent, zentrix, ModelCategory, NormalRequest,
...
use zen_api::util;
use zen_api::video::parse_video_url;
...
fn history_file_path() -> PathBuf {
    ...
    let proj_dirs = ProjectDirs::from("com", "", "Zentrix")
...
pub async fn interactive_mode(
    ...
    match zentrix.get_model_category(None) {
        Ok(ModelCategory::Text) => {
            text_interactive_mode(zentrix, do_search, enable_thinking).await
        }
        Ok(ModelCategory::Multimodal { .. }) => {
            multimodal_interactive_mode(zentrix, do_search, enable_thinking).await
        }
        Ok(ModelCategory::Diffusion) => diffusion_interactive_mode(zentrix, do_search).await,
        Ok(ModelCategory::Audio) => {
            audio_interactive_mode(zentrix, do_search, enable_thinking).await
        }
        Ok(ModelCategory::Speech) => speech_interactive_mode(zentrix, do_search).await,
...
async fn multimodal_interactive_mode(
    ...
    let config = zentrix.config(None).unwrap();
...
async fn audio_interactive_mode(
    ...
    let sender = zentrix.get_sender(None).unwrap();
    let mut rl = rustyline::DefaultEditor::new().expect("Failed to open input");
    let _ = rl.load_history(&history_file_path());

    println!(
        "{}{}{}",
        "=".repeat(20),
        "\nAudio Interactive Mode\nType a path to an audio file (.wav, .mp3) to process it, or a text prompt to chat.\nType /exit to quit.\n",
        "=".repeat(20)
    );

    let mut messages: Vec<IndexMap<String, Either<String, Vec<IndexMap<String, Value>>>>> = Vec::new();
    ...
    loop {
        ...
        let mut audios = Vec::new();
        let mut text_content = line.to_string();

        if line.ends_with(".wav") || line.ends_with(".mp3") {
            let path = std::path::PathBuf::from(line);
            if path.exists() {
                if let Ok(audio) = zen_multimodal::audio::AudioInput::read_wav(path.to_str().unwrap_or("")) {
                    audios.push(audio);
                    text_content = format!("Please transcribe or describe this audio: {}", line);
                    println!("Loaded audio file: {}", line);
                } else {
                    println!("Error loading audio file.");
                    continue;
                }
            } else {
                println!("File not found: {}", line);
                continue;
            }
        }
        ...
        user_message.insert("role".to_string(), Either::Left("user".to_string()));
        user_message.insert("content".to_string(), Either::Left(text_content));
        messages.push(user_message);

        let (tx, mut rx) = tokio::sync::mpsc::channel(100);
        
        let mut nreq = NormalRequest::new_simple(
            ...
            interactive_sample_parameters(),
            tx,
            zentrix.next_request_id(),
            None,
            None,
        );
        nreq.is_streaming = true;
        
        let req = Request::Normal(Box::new(nreq));
        ...
        let mut asst_msg = IndexMap::new();
        asst_msg.insert("role".to_string(), Either::Left("assistant".to_string()));
        asst_msg.insert("content".to_string(), Either::Left(assistant_output));
        messages.push(asst_msg);
    }
}

async fn diffusion_interactive_mode(zentrix: Arc<zentrix>, do_search: bool) {
    let sender = zentrix.get_sender(None).unwrap();
...
async fn speech_interactive_mode(zentrix: Arc<zentrix>, do_search: bool) {
    let sender = zentrix.get_sender(None).unwrap();
