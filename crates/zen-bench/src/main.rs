...
use zen_core::{
    ...
    DeviceMapSetting, DrySamplingParams, Loader, LoaderBuilder, MemoryGpuConfig, zentrix,
    ZenBuilder, ModelSelected, NormalRequest, PagedAttentionConfig, PagedCacheType, Request,
...
async fn warmup_run(zentrix: Arc<zentrix>) {
    let sampling_params = SamplingParams {
        max_len: Some(1),
...
async fn main() -> anyhow::Result<()> {
    ...
    warn!(
        "zentrix-bench is deprecated. Please use `zentrix bench` from zentrix-cli instead."
    );
    ...
    let zentrix = ZenBuilder::new(pipeline, scheduler_config, false, None)
    ...
    warmup_run(zentrix.clone()).await;
