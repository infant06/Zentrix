...
/// runtime (e.g. running a zen-vector binary built without the cargo
...
pub fn search(
    ...
    {
        let q_ref = faer::mat::from_row_major_slice::<f32>(queries, nq, dim);
        let r_ref = faer::mat::from_row_major_slice::<f32>(rotation, dim, dim);
        let out_mut = faer::mat::from_row_major_slice_mut::<f32>(&mut q_rot, nq, dim);
