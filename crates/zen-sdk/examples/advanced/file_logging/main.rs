...
//!
//! Run with: `cargo run --release --example file_logging -p zentrix`

use anyhow::Result;
use zen_sdk::{IsqBits, ModelBuilder, TextMessageRole, TextMessages};
use std::fs;
...
fn init_logging(log_file: &str) {
    ...
    let filter = EnvFilter::builder()
        ...
        .from_env_lossy()
        .add_directive("zen_core=debug".parse().unwrap());
