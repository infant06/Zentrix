...
MISTRALRS_CLI_PACKAGE="zentrix-cli"
...
install_mistralrs() {
    ...
    if [ -n "$features" ]; then
        info "Installing zentrix-cli from GitHub branch $MISTRALRS_BRANCH with features: $features"
        cargo install --force --git "$MISTRALRS_REPO_URL" --branch "$MISTRALRS_BRANCH" "$MISTRALRS_CLI_PACKAGE" --features "$features"
    else
        info "Installing zentrix-cli from GitHub branch $MISTRALRS_BRANCH with default features"
...
main() {
    ...
    echo "==========="
    echo ""
    echo "  zentrix run -m Qwen/Qwen3-4B"
    echo ""
    echo "  zentrix serve --agent -m google/gemma-4-E4B-it"
    echo ""
    ...
    printf "${YELLOW}Note:${NC} To use 'zentrix' now, run: ${BOLD}. \"\$HOME/.cargo/env\"${NC}\n"
