...
use zen_kernels_quant::RingConfig;
...
/// Engine instructions, per Engine (zentrix) ID.
...
impl Engine {
    ...
    fn replicate_request_to_daemons(&self, request: &Request) {
        if !distributed::is_daemon() && zen_kernels_quant::distributed::use_nccl() {
            let name = distributed::ipc_name().unwrap();
            let num_workers =
                zen_kernels_quant::distributed::get_global_tp_size_from_devices().unwrap() - 1;
