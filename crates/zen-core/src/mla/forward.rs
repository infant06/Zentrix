...
#[cfg(all(feature = "cuda", target_family = "unix"))]
const ZENTRIX_NO_MLA: &str = "ZENTRIX_NO_MLA";
#[cfg(all(feature = "cuda", target_family = "unix"))]
const zenllm_NO_MLA: &str = "zenllm_NO_MLA";

/// Check if MLA is disabled via environment variable.
#[cfg(all(feature = "cuda", target_family = "unix"))]
fn is_mla_disabled() -> bool {
    crate::utils::env::is_flag_enabled(ZENTRIX_NO_MLA, zenllm_NO_MLA)
}

/// Check if MLA decode should be used for single-token generation.
///
/// MLA decode is used when:
/// - `ZENTRIX_NO_MLA` (or legacy `zenllm_NO_MLA`) is not set to "1"
...
pub fn mla_decode_forward(
    ...
    kv_b_proj: &dyn zen_kernels_quant::QuantMethod,
    ...
    zen_kernels_paged::concat_and_cache_mla(
        &ckv_flat,
        &k_pe_flat,
        key_cache,
        value_cache,
    ...
    let attn_latent = zen_kernels_paged::flashinfer_mla_decode(
        &ql_nope,
        &q_pe,
        key_cache,
        value_cache,
...
pub fn mla_decode_forward(
    ...
    _kv_b_proj: &dyn zen_kernels_quant::QuantMethod,
...
pub fn mla_cache_forward(
    ...
    if !needs_prefix && !matches!(attention_mask, AttentionMask::None) {
        ...
        if total_prefix_tokens > 0 {
            ...
            let (ckv_prefix, kpe_prefix) = zen_kernels_paged::gather_mla_cache(
                key_cache,
                value_cache,
                &block_tables_seq,
                &cu_seq_lens,
