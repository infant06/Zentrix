...
use zen_core::{auto_tune, AutoTuneRequest, FitStatus, ModelSelected, QualityTier};
...
pub async fn run_tune(
    ...
    let _device_layers = device_opts.device_layers.clone();
    ...
    let requested_isq = requested
        .as_deref()
        .map(|s| {
            zen_core::parse_isq_value(s, None)
    ...
    println!("-------------------");
    if let Some(ref path) = emit_config {
        println!("  zentrix from-config --file {}", path.display());
        println!();
...
fn emit_toml_config(
    ...
    result: &zen_core::AutoTuneResult,
) -> Result<String> {
