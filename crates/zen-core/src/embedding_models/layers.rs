...
impl Module for Pooling {
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if self.pooling_mode_cls_token {
            candle_core::bail!("CLS token pooling mode is not yet supported")
        }
        ...
        if self.pooling_mode_max_tokens {
            candle_core::bail!("Max token pooling mode is not yet supported")
        }
        if self.pooling_mode_mean_sqrt_len_tokens {
            candle_core::bail!("Mean-sqrt-len token pooling mode is not yet supported")
        }
        if self.pooling_mode_weightedmean_tokens {
            candle_core::bail!("Weighted-mean token pooling mode is not yet supported")
        }
