...
$MistralRsCliPackage = "zentrix-cli"
...
# Install zentrix-cli
function Install-zentrix {
    param([string]$Features)

    if ($Features) {
        Write-Info "Installing zentrix-cli from GitHub branch $MistralRsBranch with features: $Features"
        & cargo install --force --git $MistralRsRepoUrl --branch $MistralRsBranch $MistralRsCliPackage --features "$Features"
    } else {
        Write-Info "Installing zentrix-cli from GitHub branch $MistralRsBranch with default features"
...
function Main {
    ...
    Write-Host ""
    Install-zentrix -Features $features
    ...
    Write-Host "===========" -ForegroundColor White
    Write-Host ""
    Write-Host "  zentrix run -m Qwen/Qwen3-4B"
    Write-Host ""
    Write-Host "  zentrix serve --agent -m google/gemma-4-E4B-it"
    Write-Host ""
    Write-Host "For more information, visit: https://github.com/EricLBuehler/mistral.rs"
    Write-Host ""
    Write-Host "Note: " -ForegroundColor Yellow -NoNewline
    Write-Host "Restart your terminal to use the 'zentrix' command."
}
