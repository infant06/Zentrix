...
use zen_isolate::{detect, NetworkMode, SandboxPolicy};
use tokio::process::Command;

fn workdir() -> PathBuf {
    let p = std::env::temp_dir().join(format!("zentrix-sandbox-test-{}", std::process::id()));
...
fn seccomp_available(sb: &dyn zen_isolate::Sandbox) -> bool {
    let mut policy = base_policy();
    policy.network = NetworkMode::None;
