//! ## Zentrix instance for server builder.
...
use zen_core::{
    ...
    ZenBuilder, ModelLoaderConfig, ModelSelected, MtpConfig, PagedAttentionConfig,
    PagedCacheType, SchedulerConfig, SearchCallback, SearchEmbeddingModel, TokenSource,
};
use tracing::{debug, info, warn};

use crate::types::{LoadedPipeline, SharedZentrixState};
...
pub mod defaults {
    ...
    pub const TOKEN_SOURCE: zen_core::TokenSource = zen_core::TokenSource::CacheToken;
    pub const SEARCH_CALLBACK: Option<Arc<zen_core::SearchCallback>> = None;
    pub const PAGED_CACHE_TYPE: PagedCacheType = PagedCacheType::Auto;
    pub const MTP_CONFIG: Option<zen_core::MtpConfig> = None;
}

/// A builder for creating a Zentrix instance with configured options for the Zentrix server.
///
/// ### Examples
///
/// Basic usage:
...
/// let args = Args::parse();
///
/// let zentrix = ZentrixForServerBuilder::new()
...
///        .await?;
/// ```
pub struct ZentrixForServerBuilder {
    ...
    /// Optional vector search provider to integrate Zen Vector storage
    vector_search_provider: Option<Arc<dyn zen_core::search::vector::VectorSearchProvider>>,

    /// Python code execution configuration
    code_exec_config: Option<zen_core::CodeExecutionConfig>,

    /// Mode for execution
    mode: zen_core::RuntimeMode,
}

impl Default for ZentrixForServerBuilder {
    ...
    fn default() -> Self {
        Self {
            ...
            mode: zen_core::RuntimeMode::Auto,
        }
    }
}

impl ZentrixForServerBuilder {
    /// Creates a new `ZentrixForServerBuilder` with default settings.
    ///
    ...
    /// use zen_api::zentrix_for_server_builder::ZentrixForServerBuilder;
    ///
    /// let builder = zen_api::zentrix_for_server_builder::ZentrixForServerBuilder::new();
    /// ```
    pub fn new() -> Self {
        Default::default()
    }
    ...
    /// Sets the vector search provider for this builder.
    pub fn with_vector_search_provider(
        mut self,
        provider: Arc<dyn zen_core::search::vector::VectorSearchProvider>,
    ) -> Self {
    ...
    pub fn with_code_exec_config(mut self, config: zen_core::CodeExecutionConfig) -> Self {
    ...
    pub fn with_code_exec_config_optional(
        mut self,
        config: Option<zen_core::CodeExecutionConfig>,
    ) -> Self {
    ...
    pub fn with_mode(mut self, mode: zen_core::RuntimeMode) -> Self {
        self.mode = mode;
        self
    }

    /// Builds the configured Zentrix instance.
    ///
    /// ### Examples
    ///
    /// ```ignore
    /// use zen_api::zentrix_for_server_builder::ZentrixForServerBuilder;
    ///
    /// let shared_zenllm = ZentrixForServerBuilder::new()
    ///     .with_model(model)
    ...
    ///     .await?;
    /// ```
    pub async fn build(self) -> Result<SharedZentrixState> {
    ...
    async fn build_single_model(mut self) -> Result<SharedZentrixState> {
        ...
        if let Some(mtp_config) = self.mtp_config.clone() {
            pipeline
                .lock()
                .await
                .attach_speculative(zen_core::SpeculativeConfig::Mtp(mtp_config))?;
        }
        ...
        let mut builder = ZenBuilder::new(
            pipeline,
            scheduler_config,
        ...
        if let Some(provider) = self.vector_search_provider.clone() {
            builder = builder.with_vector_search_provider(provider);
        }
    ...
    pub async fn build_multi_model(mut self) -> Result<SharedZentrixState> {
        if self.models.is_empty() {
        ...
        // Create the first zentrix instance with the first model
        let mut builder = ZenBuilder::new(
            pipeline,
        ...
        for model_config in self.models.iter().skip(1) {
            ...
            // Add the model to the zentrix instance
            let engine_config = zen_core::EngineConfig {
            ...
            let mut add_model_config = zen_core::AddModelConfig::new(engine_config);
            if let Some(mcp_config) = self.mcp_client_config.clone() {
...
fn init_device(force_cpu: bool, seed: Option<u64>) -> Result<candle_core::Device> {
    ...
    let device = if force_cpu {
        Device::Cpu
    } else if zen_core::distributed::use_nccl() {
        Device::Cpu
    } else {
...
fn zentrix_instance_info(loader: &dyn Loader) {
    debug!(
...
fn configure_paged_attn(device: &Device, paged_attn: Option<bool>) -> bool {
    ...
    } else if device.is_cuda() || zen_core::distributed::use_nccl() {
