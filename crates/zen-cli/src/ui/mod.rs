...
use ::zen_sdk::{Model, SearchEmbeddingModel};
use zen_core::{zentrix, ModelCategory, SupportedModality};
use zen_api::route_registry::{RouteInfo, RouteKind};
use tokio::fs;
...
fn build_model_list(zentrix: &Arc<zentrix>) -> IndexMap<String, UiModelInfo> {
    let mut models = IndexMap::new();
    if let Ok(list) = zentrix.list_models() {
        for model_id in list {
            if let Ok(category) = zentrix.get_model_category(Some(&model_id)) {
                ...
                if matches!(kind, "text" | "multimodal" | "speech") {
                    let cfg = zentrix.config(Some(&model_id)).ok();
...
pub async fn build_ui_router(
    ...
    let models = build_model_list(&zentrix);
    let model_wrapper = Model::new(zentrix.clone());
