...
use zen_api::zentrix_for_server_builder::{defaults, ZentrixForServerBuilder};
...
pub async fn run_quantize(model_type: QuantizeModelType, global: GlobalOptions) -> Result<()> {
    ...
    let _zenllm = ZentrixForServerBuilder::new()
...
Run with [Zentrix](https://github.com/EricLBuehler/Zentrix). Documentation: [UQFF docs](https://ericlbuehler.github.io/Zentrix/guides/perf/use-uqff/).
...
Install [Zentrix](https://github.com/EricLBuehler/Zentrix) ([full guide](https://ericlbuehler.github.io/Zentrix/guides/install/)):

**Linux/macOS:**
```
curl --proto '=https' --tlsv1.2 -sSf https://raw.githubusercontent.com/EricLBuehler/Zentrix/master/install.sh | sh
```

**Windows (PowerShell):**
```
irm https://raw.githubusercontent.com/EricLBuehler/Zentrix/master/install.ps1 | iex
```
...
"#
    ...
    for (prefix, paths) in &groups {
        ...
        output += &format!(
            "|{quant_name}|`zentrix run -m {repo_display}{model_type_flag} --from-uqff {first_file}`|\n"
        );
    }
