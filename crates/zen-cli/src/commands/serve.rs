...
use zen_api::{
    ...
    zentrix_for_server_builder::ZentrixForServerBuilder,
    zentrix_server_router_builder::ZentrixServerRouterBuilder,
    route_registry::{RouteInfo, RouteKind, ZENTRIX_API_ROUTES},
};
...
pub async fn run_server(
    ...
    let model_id_str = model_id_of(&model_type);
    let mut metadata = zen_modelhub::metadata::estimate_model_metadata(model_id_str, "f16");
    
    // Determine format to pass to metadata
    metadata.format = match &model_selected {
        ModelSelected::GGUF { .. } | ModelSelected::LoraGGUF { .. } | ModelSelected::XLoraGGUF { .. } => "gguf".to_string(),
        ModelSelected::GGML { .. } | ModelSelected::LoraGGML { .. } | ModelSelected::XLoraGGML { .. } => "ggml".to_string(),
    ...
    match decision {
        zen_orchestrator::planner::PlannerDecision::Accepted(plan) => {
            ...
            // Map selected mode back to RuntimeModeArg
    ...
    print_runtime_diagnostics(device_opts, &runtime);
    ...
    let zentrix_for_ui = zentrix.clone();

    // Build and run the server
    let mut app = ZentrixServerRouterBuilder::new()
        .with_zentrix(zentrix)
...
pub(crate) fn log_api_surfaces(host: &str, port: u16) {
    ...
    log_routes(ZENTRIX_API_ROUTES, RouteKind::OpenAi);
    debug!("Available Anthropic-compatible routes:");
    log_routes(ZENTRIX_API_ROUTES, RouteKind::Anthropic);
    debug!("Available additional Zentrix routes:");
    log_routes(ZENTRIX_API_ROUTES, RouteKind::Zentrix);
}
                ...
                ModelFormat::Gguf | ModelFormat::Ggml => {
                    ...
                    if format.quantized_file.is_none() {
                        ...
                        anyhow::bail!(
                            ...
                            Usage: zentrix run auto -m <hf model/local dir> --format {fmt} -f <filename>\n\n\
...
pub(crate) fn print_runtime_diagnostics(device_opts: &crate::args::DeviceOptions, runtime: &crate::args::RuntimeOptions) {
    let requested = format!("{:?}", runtime.mode).to_lowercase();
    let selected = if device_opts.cpu { "cpu-only".to_string() } else { requested.clone() };
    let device_sel = if device_opts.cpu { "CPU" } else { "Auto" };

    tracing::info!("--- Runtime Diagnostics ---");
    tracing::info!("Requested mode : {}", requested);
    tracing::info!("Selected mode  : {}", selected);
    tracing::info!("GPU layers     : {}", runtime.gpu_layers);
    tracing::info!("Fallback allowed: {}", runtime.allow_fallback);
    tracing::info!("Fallback reason : None");
    tracing::info!("Device selected: {}", device_sel);
    tracing::info!("---------------------------");
}

pub(crate) fn extract_device_settings(model_type: &ModelType) -> &crate::args::DeviceOptions {
    match model_type {
...
pub(crate) async fn apply_quant_resolution(
    model_type: &mut ModelType,
    token_source: &zen_core::TokenSource,
    matformer: &MatformerSelection,
) -> Result<()> {
    ...
    let force_cpu = extract_device_settings(model_type).cpu;
    let model_selected = convert_to_model_selected(model_type, matformer)?;
...
pub(crate) fn build_code_exec_config(
    ...
    sandbox_policy: Option<zen_isolate::SandboxPolicy>,
) -> Option<zen_core::CodeExecutionConfig> {
    if !runtime.enable_code_execution {
        return None;
    }
    let mut config = zen_core::CodeExecutionConfig::default();
...
pub(crate) fn extract_sandbox_settings(
    sandbox: SandboxOptions,
) -> Option<zen_isolate::SandboxPolicy> {
    let mode = match (
        sandbox.mode,
        std::env::var(zen_isolate::SANDBOX_ENV_VAR).ok(),
    ) {
    ...
    match mode {
        ...
        SandboxMode::Auto | SandboxMode::On => {
            let mut policy = zen_isolate::SandboxPolicy::default();
...
pub(crate) fn log_agent_runtime(runtime: &RuntimeOptions, max_tool_rounds: Option<usize>) {
    ...
    let rounds = max_tool_rounds.unwrap_or(zen_core::DEFAULT_MAX_TOOL_ROUNDS);
...
fn search_summary(runtime: &RuntimeOptions) -> String {
    ...
    let model = runtime
        ...
        .map(zen_core::SearchEmbeddingModel::from)
        .unwrap_or_default();
