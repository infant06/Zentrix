...
impl DeviceMappedModelLoader for GgufDeviceMapLoaderInner<'_, '_> {
    ...
    fn non_mapped_size_in_bytes(
        ...
        let size_in_bytes = match self.arch {
            ...
            _ => anyhow::bail!("Unsupported GGUF architecture {:?} for non-layer size calculation", self.arch),
        };
        Ok(size_in_bytes)
    }
    ...
    fn layer_sizes_in_bytes(
        ...
        let size_in_bytes = match self.arch {
            ...
            _ => anyhow::bail!("Unsupported GGUF architecture {:?} for layer size calculation", self.arch),
        };
