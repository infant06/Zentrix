...
//!
//! Ported from zentrix-server/src/interactive_mode.rs
...
use zen_core::{
    ...
    DrySamplingParams, ImageGenerationResponseFormat, MessageContent, zentrix, ModelCategory,
...
fn history_file_path() -> PathBuf {
    ...
    // e.g. ~/.config/Zentrix/history.txt
...
async fn oneshot_text(
    ...
    do_code_exec: bool,
    agent_permission: AgentPermission,
    agent_approval_callback: Option<zen_core::AgentToolApprovalCallback>,
    enable_thinking: Option<bool>,
    text: String,
) {
    let sender = zentrix.get_sender(None).unwrap();
    let sampling_params = interactive_sample_parameters(&zentrix);
    ...
    let req = Request::Normal(Box::new(NormalRequest {
        ...
        agent_approval_handler: agent_approval_callback
            .map(zen_core::AgentToolApprovalHandler::from_sync),
...
fn print_stats(
    ...
    if let Some(last_usage) = last_usage {
        ...
        if let Some(ttft) = first_token_duration {
            ...
            zen_telemetry::Telemetry::record_ttft(ttft);
        }
        println!(
            "Prompt: {} tokens, {:.2} T/s",
        ...
        zen_telemetry::Telemetry::record_tps(last_usage.avg_compl_tok_per_sec as f64);
        if let Ok(logger) = zentrix.get_logger(None) {
...
fn interactive_sample_parameters(zentrix: &Arc<zentrix>) -> SamplingParams {
    match zentrix
        .config(None)
        .ok()
...
async fn text_interactive_mode(
    ...
    let mut sampling_params = interactive_sample_parameters(&zentrix);
...
fn print_agentic_progress(
    tool_name: &str,
    phase: &zen_core::AgenticToolCallPhase,
    files: &[zen_core::files::File],
) {
    use zen_core::{AgenticToolCallData, AgenticToolCallPhase};
...
pub(super) fn agent_approval_callback() -> zen_core::AgentToolApprovalCallback {
    std::sync::Arc::new(move |approval: &zen_core::AgentToolApproval| {
        let _render_guard = AGENTIC_RENDER_LOCK.lock().unwrap();
        ...
        loop {
            ...
            if io::stdin().read_line(&mut input).is_err() {
                return zen_core::AgentToolApprovalDecision::deny(None);
            }
            match input.trim().to_ascii_lowercase().as_str() {
                "y" | "yes" => return zen_core::AgentToolApprovalDecision::approve(),
                "a" | "always" => {
                    return zen_core::AgentToolApprovalDecision::approve_for_session();
                }
                "" | "n" | "no" => {
                    return zen_core::AgentToolApprovalDecision::deny(None);
                }
    ...
    while let Some(resp) = rx.recv().await {
        match resp {
            ...
            Response::AgenticToolCallProgress {
                ...
                let complete = matches!(phase, zen_core::AgenticToolCallPhase::Complete(_));
...
async fn audio_interactive_mode(
    ...
    _agent_approval_callback: Option<zen_core::AgentToolApprovalCallback>,
    _enable_thinking: Option<bool>,
) {
    error!("Audio models are not supported in `zentrix run`. Use `zentrix serve` and the OpenAI-compatible /v1/chat/completions endpoint instead.");
}
