...
pub fn metal_apply_sparse_penalties(
    ...
    {
        use zen_kernels_quant::metal_kernels::Kernels;
        zen_kernels_quant::metal_kernels::call_copy_logits(
            device.device(),
            &encoder,
            &Kernels::new(),
            dtype,
    ...
    zen_kernels_quant::metal_kernels::call_apply_sparse_penalties(
        device.device(),
        &encoder,
        &zen_kernels_quant::metal_kernels::Kernels::new(),
        dtype,
        &out_buf,
        tok_s.buffer(),
        cnt_s.buffer(),
...
pub fn metal_rms_norm_residual(
    ...
    zen_kernels_quant::metal_kernels::call_rmsnorm_residual(
        device.device(),
        &encoder,
        &zen_kernels_quant::metal_kernels::Kernels::new(),
        dtype,
...
pub fn metal_topk_logits_packed(
    ...
    zen_kernels_quant::metal_kernels::call_topk_logits_packed(
        device.device(),
        &encoder,
        &zen_kernels_quant::metal_kernels::Kernels::new(),
        dtype,
...
fn glu_activation_type(act: Activation) -> Option<zen_kernels_quant::GluActivationType> {
    match act {
        Activation::Silu | Activation::Swish => Some(zen_kernels_quant::GluActivationType::Silu),
        Activation::NewGelu | Activation::GeluPytorchTanh => {
            Some(zen_kernels_quant::GluActivationType::Gelu)
        }
        Activation::Gelu => Some(zen_kernels_quant::GluActivationType::GeluErf),
        Activation::Relu => Some(zen_kernels_quant::GluActivationType::Relu),
        _ => None,
    }
}
...
) -> Option<zen_kernels_quant::GluActivationType> {
    match act {
        candle_nn::Activation::Silu | candle_nn::Activation::Swish => {
            Some(zen_kernels_quant::GluActivationType::Silu)
        }
        candle_nn::Activation::NewGelu | candle_nn::Activation::GeluPytorchTanh => {
            Some(zen_kernels_quant::GluActivationType::Gelu)
        }
        candle_nn::Activation::Gelu => Some(zen_kernels_quant::GluActivationType::GeluErf),
        candle_nn::Activation::Relu => Some(zen_kernels_quant::GluActivationType::Relu),
        _ => None,
    }
}
...
pub fn mul_and_act(a: &Tensor, b: &Tensor, act: Activation) -> Result<Tensor> {
    ...
    if matches!(a.dtype(), DType::F16 | DType::BF16 | DType::F32) && a.dtype() == b.dtype() {
        if let Some(activation_type) = glu_activation_type(act) {
            return zen_kernels_quant::fused_glu(a, b, activation_type);
        }
    }
...
pub(crate) struct MergedDenseProjection {
    proj: Arc<dyn zen_kernels_quant::QuantMethod>,
    output_dims: Vec<usize>,
}

impl MergedDenseProjection {
    pub(crate) fn new(projs: &[&dyn zen_kernels_quant::QuantMethod]) -> Result<Option<Self>> {
        ...
        if zen_kernels_quant::get_immediate_isq()
            .is_some_and(|p| p.capture != zen_kernels_quant::IsqCaptureMode::Immediate)
        {
            return Ok(None);
        }
        ...
        let proj = <zen_kernels_quant::UnquantLinear as zen_kernels_quant::QuantMethod>::new(
            zen_kernels_quant::QuantMethodConfig::Unquantized(Linear::new(weight, None)),
        )?;
...
pub(crate) fn quantized_ffn(
    xs: &Tensor,
    gate: &dyn zen_kernels_quant::QuantMethod,
    up: &dyn zen_kernels_quant::QuantMethod,
    down: &dyn zen_kernels_quant::QuantMethod,
    act: Activation,
) -> Result<Tensor> {
    #[cfg(feature = "cuda")]
    if let Some(activation_type) = glu_activation_type(act) {
        if let Some(inter) =
            zen_kernels_quant::try_fused_quantized_gate_up(xs, gate, up, activation_type)?
        {
    ...
    if let Some(activation_type) = glu_activation_type(act) {
        if let Some(inter) =
            zen_kernels_quant::try_fused_gate_up_metal(xs, gate, up, activation_type)?
        {
            return down.forward(&inter);
        }
    }
...
pub(crate) fn qkv_projections(
    xs: &Tensor,
    q_proj: &dyn zen_kernels_quant::QuantMethod,
    k_proj: &dyn zen_kernels_quant::QuantMethod,
    v_proj: &dyn zen_kernels_quant::QuantMethod,
) -> Result<(Tensor, Tensor, Tensor)> {
    #[cfg(feature = "cuda")]
    if let Some(qkv) = zen_kernels_quant::try_fused_quantized_qkv(xs, q_proj, k_proj, v_proj)? {
        return Ok(qkv);
    }

    #[cfg(feature = "metal")]
    if let Some(qkv) = zen_kernels_quant::try_fused_qkv_metal(xs, q_proj, k_proj, v_proj)? {
        return Ok(qkv);
    }
