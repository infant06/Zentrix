...
pub fn get_global_tp_size_from_devices() -> Result<usize> {
    ...
    #[cfg(all(feature = "cuda", feature = "nccl"))]
    {
        ...
        if let Ok(x) = std::env::var("zenllm_MN_LOCAL_WORLD_SIZE") {
            use std::str::FromStr;
            Ok(usize::from_str(&x).expect("Not a number for zenllm_MN_LOCAL_WORLD_SIZE!"))
        } else {
...
pub fn use_nccl() -> bool {
    (std::env::var("zenllm_NO_NCCL").is_err()
        || std::env::var("zenllm_NO_NCCL").is_ok_and(|x| x != "1"))
