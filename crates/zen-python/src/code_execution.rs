...
impl From<NetworkModePy> for zen_core::NetworkMode {
...
impl From<CodeExecutionPermissionPy> for zen_core::CodeExecutionPermission {
...
impl From<AgentPermissionPy> for zen_core::AgentPermission {
...
impl From<&zen_core::AgentToolSource> for AgentToolSourcePy {
    fn from(value: &zen_core::AgentToolSource) -> Self {
        match value {
            zen_core::AgentToolSource::BuiltIn => Self::BuiltIn,
            zen_core::AgentToolSource::User => Self::User,
            zen_core::AgentToolSource::Mcp => Self::Mcp,
            zen_core::AgentToolSource::External => Self::External,
        }
    }
}
...
impl From<&zen_core::AgentToolKind> for AgentToolKindPy {
    fn from(value: &zen_core::AgentToolKind) -> Self {
        match value {
            zen_core::AgentToolKind::CodeExecution => Self::CodeExecution,
            zen_core::AgentToolKind::WebSearch => Self::WebSearch,
            zen_core::AgentToolKind::File => Self::File,
            zen_core::AgentToolKind::Custom => Self::Custom,
            zen_core::AgentToolKind::External => Self::External,
        }
    }
}
...
/// See the [sandbox reference](/Zentrix/reference/sandbox/) for layer
/// details.
#[pyclass]
#[derive(Clone, Debug)]
pub struct SandboxPolicy {
    ...
    pub(crate) network: zen_core::NetworkMode,
...
impl SandboxPolicy {
    ...
    fn new(
        ...
        let network = parse_network_mode(network)?.unwrap_or(zen_core::NetworkMode::Loopback);
        Ok(Self {
            max_memory_mb,
            max_cpu_secs,
            max_procs,
...
impl From<SandboxPolicy> for zen_core::SandboxPolicy {
    fn from(p: SandboxPolicy) -> Self {
        zen_core::SandboxPolicy {
...
pub struct CodeExecutionConfig {
    ...
    pub(crate) permission: Option<zen_core::CodeExecutionPermission>,
...
) -> PyResult<Option<zen_core::CodeExecutionPermission>> {
    value
        .map(|value| {
            Python::with_gil(|py| {
                ...
                match value.extract::<String>()?.to_ascii_lowercase().as_str() {
                    PERMISSION_AUTO => Ok(zen_core::CodeExecutionPermission::Auto),
                    PERMISSION_ASK => Ok(zen_core::CodeExecutionPermission::Ask),
                    PERMISSION_DENY => Ok(zen_core::CodeExecutionPermission::Deny),
...
pub(crate) fn parse_agent_permission(
    value: Option<Py<PyAny>>,
) -> PyResult<Option<zen_core::AgentPermission>> {
    value
        .map(|value| {
            Python::with_gil(|py| {
                let value = value.bind(py);
                ...
                match value.extract::<String>()?.to_ascii_lowercase().as_str() {
                    PERMISSION_AUTO => Ok(zen_core::AgentPermission::Auto),
                    PERMISSION_ASK => Ok(zen_core::AgentPermission::Ask),
                    PERMISSION_DENY => Ok(zen_core::AgentPermission::Deny),
                    other => Err(PyValueError::new_err(format!(
...
fn parse_network_mode(value: Option<Py<PyAny>>) -> PyResult<Option<zen_core::NetworkMode>> {
    value
        .map(|value| {
            Python::with_gil(|py| {
                let value = value.bind(py);
                ...
                match value.extract::<String>()?.to_ascii_lowercase().as_str() {
                    NETWORK_NONE => Ok(zen_core::NetworkMode::None),
                    NETWORK_LOOPBACK => Ok(zen_core::NetworkMode::Loopback),
                    NETWORK_FULL => Ok(zen_core::NetworkMode::Full),
                    other => Err(PyValueError::new_err(format!(
...
impl From<&zen_core::AgentToolMetadata> for AgentToolMetadataPy {
    fn from(tool: &zen_core::AgentToolMetadata) -> Self {
        Self {
...
impl From<&zen_core::AgentToolApproval> for AgentToolApprovalPy {
    fn from(approval: &zen_core::AgentToolApproval) -> Self {
        Self {
...
impl AgentToolApprovalDecisionPy {
    fn to_core(&self) -> zen_core::AgentToolApprovalDecision {
        match self.decision {
            AgentToolApprovalDecisionKindPy::Approve => {
                zen_core::AgentToolApprovalDecision::approve()
                    .with_remember_for_session(self.remember_for_session)
            }
            AgentToolApprovalDecisionKindPy::Deny => {
                zen_core::AgentToolApprovalDecision::deny(self.message.clone())
            }
        }
    }
}
...
fn parse_agent_approval_result(
    py: Python<'_>,
    result: Py<PyAny>,
) -> PyResult<zen_core::AgentToolApprovalDecision> {
    if let Ok(approve) = result.extract::<bool>(py) {
        return Ok(if approve {
            zen_core::AgentToolApprovalDecision::approve()
        } else {
            zen_core::AgentToolApprovalDecision::deny(None)
        });
    }
...
) -> Option<zen_core::AgentToolApprovalCallback> {
    callback.map(|callback| {
        Arc::new(move |approval: &zen_core::AgentToolApproval| {
            Python::with_gil(|py| {
                let payload = AgentToolApprovalPy::from(approval);
                callback
                    .call1(py, (payload,))
                    .and_then(|result| parse_agent_approval_result(py, result))
                    .unwrap_or_else(|_| zen_core::AgentToolApprovalDecision::deny(None))
            })
        }) as zen_core::AgentToolApprovalCallback
    })
}

impl From<CodeExecutionConfig> for zen_core::CodeExecutionConfig {
    fn from(cfg: CodeExecutionConfig) -> Self {
        let default = zen_core::CodeExecutionConfig::default();
        let approval_callback = cfg.approval_callback.map(|callback| {
            Arc::new(move |approval: &zen_core::CodeExecutionApproval| {
                Python::with_gil(|py| {
            ...
            }) as zen_core::CodeExecutionApprovalCallback
        });
        zen_core::CodeExecutionConfig {
