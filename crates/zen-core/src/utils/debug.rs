...
pub const ZEN_LOG_TARGET_PREFIX: &str = "zentrix";
...
/// This should not be called in zentrix-runtime code due to Rust usage.
pub fn initialize_logging() {
    ...
    initialize_zentrix_logging(verbosity);
}

/// Initialize Zentrix logging with the specified verbosity level.
pub fn initialize_zentrix_logging(verbosity: LogVerbosity) {
    let filter =
        EnvFilter::try_from_default_env().unwrap_or_else(|_| default_zentrix_filter(verbosity));
    initialize_logging_with_filter(filter);
}

/// Deprecated: Use `initialize_zentrix_logging` instead.
#[deprecated(since = "0.2.0", note = "Use initialize_zentrix_logging instead")]
#[allow(dead_code)]
...
/// Create the default Zentrix log filter for the specified verbosity level.
pub fn default_zentrix_filter(verbosity: LogVerbosity) -> EnvFilter {
    ...
    EnvFilter::new(base).add_directive(
        format!("{ZEN_LOG_TARGET_PREFIX}={level}")
            .parse()
            .expect("valid default log directive"),
    )
}

/// Deprecated: Use `default_zentrix_filter` instead.
#[deprecated(since = "0.2.0", note = "Use default_zentrix_filter instead")]
#[allow(dead_code)]
pub fn default_mistralrs_filter(verbosity: LogVerbosity) -> EnvFilter {
    default_zentrix_filter(verbosity)
}

fn is_debug_env() -> bool {
    crate::utils::env::is_flag_enabled("ZENTRIX_DEBUG", "zenllm_DEBUG")
}
