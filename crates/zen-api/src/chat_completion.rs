...
use zen_core::{
    ...
    ChatCompletionResponse, Constraint, zentrix, ModelCategory, NormalRequest, ReasoningEffort,
...
use crate::{
    ...
    zentrix_server_router_builder::AgenticDefaults,
    openai::{
    ...
    types::{ExtractedZentrixState, OnChunkCallback, OnDoneCallback, SharedZentrixState},
...
///
/// ```no_run
/// use zen_api::chat_completion::ChatCompletionOnChunkCallback;
///
...
///
/// ```no_run
/// use zen_api::chat_completion::ChatCompletionOnDoneCallback;
///
...
fn stamp_file_ids(records: &mut [AgenticToolCallRecord], files: &[zen_core::File]) {
        ...
        match self.rx.poll_recv(cx) {
            Poll::Ready(Some(resp)) => match resp {
                ...
                Response::InternalError(e) => {
                    zentrix::maybe_log_error(self.state.clone(), &*e);
                    self.done_state = DoneState::SendingDone;
                    Poll::Ready(Some(Ok(
                        Event::default().data(sanitize_error_message(e.as_ref()))
                    )))
                ...
                Response::Chunk(mut response) => {
                    ...
                    // Done now, just need to send the [DONE]
                    zentrix::maybe_log_response(self.state.clone(), &response);
...
/// request format used by Zentrix.
pub async fn parse_request(
    mut oairequest: ChatCompletionRequest,
    state: SharedZentrixState,
    tx: Sender<Response>,
    ...
    zentrix::maybe_log_request(state.clone(), repr);

    // Validate that the requested model matches the loaded model
    validate_model_name(&oairequest.model, state.clone())?;

    // --- RAG Augmentation ---
    // Extract the last user message to query the vector store
    let last_user_text = match &oairequest.messages {
        either::Either::Left(msgs) => {
            msgs.iter()
                .rev()
                .find(|m| m.role == "user")
                .and_then(|m| match &m.content {
                    Some(content) => {
                        match &content.0 {
                            either::Either::Left(text) => Some(text.clone()),
                            _ => None,
                        }
                    }
                    None => None,
        ...
        either::Either::Right(prompt) => Some(prompt.clone()),
    };

    if let Some(query_text) = last_user_text {
        if let Ok(Some(context)) = state.augment_with_rag(Some(&oairequest.model), &query_text).await {
            match &mut oairequest.messages {
                either::Either::Left(msgs) => {
                    if let Some(msg) = msgs.iter_mut().rev().find(|m| m.role == "user") {
                        if let Some(ref mut content) = msg.content {
                            if let either::Either::Left(ref mut text) = content.0 {
                                *text = format!("{}\n{}", context, text);
                            }
                        }
                    }
...
pub async fn process_non_streaming_response(
    ...
    let mut files: Vec<zen_core::File> = Vec::new();
...
pub fn match_responses(state: SharedZentrixState, response: Response) -> ChatCompletionResponder {
    match response {
        Response::InternalError(e) => {
            zentrix::maybe_log_error(state, &*e);
            ChatCompletionResponder::InternalError(e)
        }
        Response::ModelError(msg, response) => {
            zentrix::maybe_log_error(state.clone(), &ModelErrorMessage(msg.to_string()));
            zentrix::maybe_log_response(state, &response);
            ChatCompletionResponder::ModelError(msg, response)
        }
