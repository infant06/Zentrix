...
pub trait DeviceMapper: Debug {
    ...
    fn get_comm_for(&self, layer_idx: usize) -> Result<Arc<zen_kernels_quant::Comm>>;
...
impl DeviceMapper for LayerDeviceMapper {
    ...
    fn get_comm_for(&self, layer_idx: usize) -> Result<Arc<zen_kernels_quant::Comm>> {
        let id = zen_kernels_quant::Id::new();
        Ok(Arc::new(zen_kernels_quant::Comm::from_device(
            id,
...
pub struct NcclDeviceMapper {
    nm_device: Device,
    model_layers: usize,
    comm: Option<Arc<zen_kernels_quant::Comm>>,
}
...
pub struct NcclPipelineParallelMapper {
    mappings: Vec<(Arc<zen_kernels_quant::Comm>, Device)>,
    nm_device: Device,
    cuda_peer_access: CudaPeerAccess,
}
