...
use zen_core::SupportedModality;
...
impl McpTool for ChatTool {
    ...
    async fn call(
        ...
        let chat_req: zen_api::openai::ChatCompletionRequest =
        ...
        zen_api::handler_core::send_request(state, request)
            .await
            .map_err(|e| CallToolError::new(io::Error::other(e.to_string())))?;

        match rx.recv().await {
            Some(zen_core::Response::Done(resp)) => {
                let content = resp
                    .choices
                    .iter()
            ...
            Some(zen_core::Response::ModelError(msg, _)) => {
...
pub struct HttpMcpHandler {
    pub state: SharedZentrixState,
    tools: HashMap<String, Arc<dyn McpTool>>,
    server_info: InitializeResult,
}

impl HttpMcpHandler {
    pub fn new(state: SharedZentrixState) -> Self {
